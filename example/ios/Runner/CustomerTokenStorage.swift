...
class CustomerTokenStorage {
    ...
    private func loadJwtToken() -> String? {
        ...
        guard let host = currentConf?.host,
            ...
            Sendsay.logger.log(.verbose, message: "CustomerTokenStorage not configured yet")
            return nil
        }
        guard let url = URL(string: "\(host)/webxp/exampleapp/customertokens") else {
            Sendsay.logger.log(.error, message: "Invalid URL host \(host) for CustomerTokenStorage")
            return nil
        }
        ...
        do {
            ...
            guard let httpResponseData = httpResponseData else {
                Sendsay.logger.log(.error, message: "Example token response is empty")
                return nil
            }
            let responseData = try JSONDecoder().decode(TokenResponse.self, from: httpResponseData)
            if responseData.token == nil {
                Sendsay.logger.log(.error, message: "Example token received NULL")
            }
            return responseData.token
        } catch let error {
            Sendsay.logger.log(.error, message: "Example token cannot be parsed due error \(error.localizedDescription)")
            return nil
        }
    }

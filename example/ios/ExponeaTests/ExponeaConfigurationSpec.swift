//
//  SendsayConfigurationParserSpec.swift
//  SendsayTests
//
...
@testable import sendsay

class SendsayConfigurationParserSpec: QuickSpec {
    override func spec() {
        describe("parse") {
            ...
            it("minimal") {
                ...
                expect(settings.baseUrl).to(equal(SendsaySDK.Constants.Repository.baseUrl))
                
                let setup = config.flushingSetup
                expect(setup.maxRetries).to(equal(SendsaySDK.Constants.Session.maxRetries))
                
                let sessionTracking = config.automaticSessionTracking
                expect(sessionTracking.enabled).to(equal(true))
                expect(sessionTracking.timeout).to(equal(SendsaySDK.Constants.Session.defaultTimeout))

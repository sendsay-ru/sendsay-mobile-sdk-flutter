...
import com.sendsay.data.FlutterContentBlockCarouselSelector
...
class FlutterInAppContentBlockCarousel(
    ...
    companion object {
        private const val CHANNEL_NAME = "ru.sendsay/InAppContentBlockCarousel"
                    ...
                    filterCallback = if (filtrationSet) { source ->
                        CoroutineScope(Dispatchers.Main).launch {
                            channel?.invokeMethod(
                                ...
                                source.map { SendsayGson.instance.toJson(it) }
                            )
                        }
                    } else null,
            ...
            inAppContentBlockCarousel.behaviourCallback = object : ContentBlockCarouselCallback {
                ...
                override fun onActionClicked(
                    ...
                    val payload: Map<String, Any?> = mapOf(
                        ...
                        "contentBlock" to SendsayGson.instance.toJson(contentBlock),
                ...
                override fun onMessagesChanged(count: Int, messages: List<InAppContentBlock>) {
                    val payload: Map<String, Any?> = mapOf(
                        ...
                        "count" to count,
                        "messages" to messages.map { SendsayGson.instance.toJson(it) }
                    )
                    invokeMethod(METHOD_ON_IN_APP_CONTENT_BLOCK_CAROUSEL_EVENT, payload)
                }
    ...
    override fun onMethodCall(call: MethodCall, result: Result) {
        when (call.method) {
            METHOD_FILTER_CONTENT_BLOCKS_RESULT -> {
                val source = (call.arguments as List<Any?>).map { SendsayGson.instance.fromJson(it as String, InAppContentBlock::class.java) }

...
abstract class BaseInterface {
  /// Configure Sendsay SDK.
  /// Should only be called once.
  /// You need to configure SendsaySDK before calling most methods.
  /// Returns true if configuration was successful. Returns false if sdk was already configured.
  Future<bool> configure(SendsayConfiguration configuration);

  /// Check whether Sendsay SDK is configured.
  ...
  /// Get the period with which events are tracked to Sendsay backend.
  /// Only valid in [FlushMode.period].
  Future<Duration> getFlushPeriod();

  /// Set the period with which events are tracked to Sendsay backend.
  /// Only valid in [FlushMode.period].
  ...
  /// Push token is cleared on Sendsay backend.
  /// Optionally changes default Sendsay project and event-project mapping.

...
/// An implementation of [SendsayPlatform] that uses method channels.
class MethodChannelSendsayPlatform extends SendsayPlatform {
  static const _channelName = "com.sendsay";
      // "${Platform.isAndroid ? 'com' : 'ru'}.sendsay.example";
  ...
  Future<bool> configure(SendsayConfiguration configuration) async {
    final data = SendsayConfigurationEncoder.encode(configuration);
  ...
  Future<void> anonymize([
    SendsayConfigurationChange configChange =
        const SendsayConfigurationChange(),
  ]) async {
    final data = SendsayConfigurationChangeEncoder.encode(configChange);

...
Flutter applications code can be reloaded without restarting the native application itself. This speeds up the development process, but it also means that native code usually continues to run as if nothing happened. You should only configure the SDK once. When developing with hot reload enabled, you should check `SendsayPlugin().isConfigured()` before configuring the SDK.
...
When Xcode asks you if you want to create bridging headers, we recommend you do so, since a missing bridging header can lead to compilation errors when using Xcode 12.5 (for example, [in this GitHub issue](https://github.com/sendsay/sendsay-react-native-sdk/issues/19)).

After cleaning the project, a build should succeed.

> 📘
>
> [Check similar issue on GitHub](https://github.com/sendsay/sendsay-react-native-sdk/issues/12)

### SWIFT_VERSION not specified (iOS-specific)

If `SWIFT_VERSION` is not set, there may be some Swift compilation errors in the Sendsay iOS SDK in the Swift files.
...
- **SDK initialization sequence:** 
  Ensure that the `SendsayPlugin`'s `configure()` method is called at an appropriate point in your app's initialization sequence. It should ideally be called early in the app's lifecycle, possibly in the `main()` function before `runApp()`. Ensure it's not being delayed or called conditionally in a way that might cause it to be skipped.
- **Dependency Initialization**:
  Check for any missing dependencies or prerequisites that the Sendsay SDK relies on for initialization. Properly initialize these dependencies before calling the `SendsayPlugin`'s `configure()` method.
- **Error Logging:**
  Implement error logging and handling in your app to capture any potential errors or exceptions that might occur during the initialization of the Sendsay SDK. This can help you pinpoint the root cause of the issue.
- **SDK Version:**

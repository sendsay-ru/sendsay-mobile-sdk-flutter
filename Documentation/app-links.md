...
The official [Flutter documentation](https://docs.flutter.dev/ui/navigation/deep-linking) describes how to set up your application and how to process incoming links. , we just need to add tracking to Sendsay.
...
Your `AppDelegate` must call `SwiftSendsayPlugin.continueUserActivity(userActivity)` to make Universal Links work.

#### With SendsayFlutterAppDelegate

If your `AppDelegate` already extends `SendsayFlutterAppDelegate`, no change is required.

#### Without SendsayFlutterAppDelegate

If you don't use the `SendsayFlutterAppDelegate`, you must implement the method and call `SwiftSendsayPlugin`:

...
To use real-time segments in your app, you must listen to one or more customized stream instances. to get these streams, you need to call `SendsayPlugin().segmentationDataStream` method. This methods takes two arguments:
...
The SDK provides an API to get segmentation data directly. Invoke the `SendsayPlugin().getSegments` method, passing a `exposingCategory` value as argument:
...
3. When `SendsayPlugin().identifyCustomer` is called with a [hard ID](https://documentation.bloomreach.com/engagement/docs/customer-identification#section-hard-id).
...
If you are not receiving segmentation data while registering a customer, please check your usage of `SendsayPlugin().identifyCustomer` or `SendsayPlugin().anonymize`. You may face these logs:

- ```
  Segments: Segments change check has been cancelled meanwhile
  ```
  The segmentation data update process started but was subsequently canceled by an invocation of `SendsayPlugin().anonymize`. If this is unwanted behavior, check your `SendsayPlugin().anonymize` usage.
- ```
  Segments: Check process was canceled because customer has changed
  ```
  The segmentation data update process started for the current customer but the customer ID was subsequently changed by an invocation of `SendsayPlugin().identifyCustomer` for another customer. If this is unwanted behaviour, check your `SendsayPlugin().identifyCustomer` usage.
- ```
  Segments: Customer IDs <customer_ids> merge failed, unable to fetch segments
  ```
  The segmentation data update process requires to link IDs but that part of the process failed. Please refer to the error log messages and check your `SendsayPlugin().identifyCustomer` usage. This  should not happen, please discuss this with the Bloomreach support team.
- ```
  Segments: New data are ignored because were loaded for different customer
  ```
  The segmentation data update process detects that data has been fetched for a previous customer. This should not lead to any problem as there is another fetch process registered for the new customer, but you may face a short delay for new data retrieval. If you see this log often, check your `SendsayPlugin().shared.identifyCustomer` usage.
- ```

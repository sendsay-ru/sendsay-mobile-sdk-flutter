...
You can override the SDK's default behavior in response to an in-app message action (click button or close message) by setting up a listener for `inAppMessageAction` using `SendsayPlugin.inAppMessageActionStream({bool overrideDefaultBehavior = false, bool trackActions = true})`.
...
- The SDK requests in-app messages from the Engagement platform any time one of the following occurs:
  - `SendsayPlugin.identifyCustomer` is called
  - `SendsayPlugin.anonymize` is called
...
- If you create or modify an in-app message in Engagement, typically any changes you made are reflected in the SDK after 30 minutes due to the SDK caching in-app messages. Call `SendsayPlugin.identifyCustomer` or `SendsayPlugin.anonymize` to trigger reloading so changes are reflected immediately.
...
> ❗️
>
> Invoking `SendsayPlugin.anonymize` triggers fetching in-app messages immediately but `SendsayPlugin.identifyCustomer` needs to be flushed to the backend successfully first. This is because the backend must know the customer so it can assign the in-app messages with matching audience. If you have set [flush mode](https://documentation.bloomreach.com/engagement/docs/flutter-sdk-data-flushing#flushing-modes) to anything other then `FlushMode.immediate`, you must call `SendsayPlugin.flushData()` to finalize the `identifyCustomer` process and trigger an in-app messages fetch.

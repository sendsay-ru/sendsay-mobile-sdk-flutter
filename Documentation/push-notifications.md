...
To respond to a push notification interaction, you can set up a listener to `SendsayPlugin().openedPushStream`:
...
You can set up a listener for received push notifications using `SendsayPlugin.receivedPushStream`, which is especially useful for silent push notifications.
...
If the provided native `SendsayModule.Companion.handleRemoteMessage` (Android) and `SendsayNotificationService().process` (iOS)  methods don't fit the requirements of your app, or you decide to disable automatic push notifications, you must handle push notifications and process their payload yourself.

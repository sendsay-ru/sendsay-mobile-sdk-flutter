...
![App Inbox button](https://raw.githubusercontent.com/sendsay/sendsay-flutter-sdk/main/Documentation/images/app-inbox-button.png)
...
> 📘
>
> See [home.dart](https://github.com/sendsay/sendsay-flutter-sdk/blob/main/example/lib/page/home.dart) for a reference implementation.
...
3. Call `SendsayPlugin().trackAppInboxOpened` when the user clicks on a list item and mark the message as read automatically.
...
5. Call `SendsayPlugin().trackAppInboxClick` automatically when the user clicks a button in the message detail view.


![App Inbox messages list view and message detail view](https://raw.githubusercontent.com/sendsay/sendsay-flutter-sdk/main/Documentation/images/app-inbox-flutter.png)
...
```xml
<string name="sendsay_inbox_button">Inbox</string>
<string name="sendsay_inbox_title">Inbox</string>
<string name="sendsay_inbox_defaultTitle">Inbox message</string>
<string name="sendsay_inbox_emptyTitle">Empty Inbox</string>
<string name="sendsay_inbox_emptyMessage">You have no messages yet.</string>
<string name="sendsay_inbox_errorTitle">Something went wrong :(</string>
<string name="sendsay_inbox_errorMessage">We could not retrieve your messages.</string>
<string name="sendsay_inbox_mainActionTitle">See more</string>
```

For iOS, add to your `Localizable.string` files:

```text
"sendsay.inbox.button" = "Inbox";
"sendsay.inbox.title" = "AppInbox";
"sendsay.inbox.loading" = "Loading messages...";
"sendsay.inbox.emptyTitle" = "Empty Inbox";
"sendsay.inbox.emptyMessage" = "You have no messages yet.";
"sendsay.inbox.errorTitle" = "Something went wrong :(";
"sendsay.inbox.errorMessage" = "We could not retrieve your messages.";
"sendsay.inbox.defaultTitle" = "Message";
"sendsay.inbox.mainActionTitle" = "See more";
```
...
The methods `SendsayPlugin().trackAppInboxOpened` and `SendsayPlugin().markAppInboxAsRead` are called automatically when the user clicks on an item.
...
The method `SendsayPlugin().trackAppInboxClick` is called automatically when the user clicks on an action.
...
- `SendsayPlugin().identifyCustomer`
- `SendsayPlugin().anonymize`
...
```dart
var messages = await SendsayPlugin().fetchAppInbox();
```

It's also possible to fetch a single message by its ID from the App Inbox as follows:

```dart
var message = await SendsayPlugin().fetchAppInboxItem(messageId);
```
...
```dart
var markedAsRead = SendsayPlugin().markAppInboxAsRead(message);
```

> ❗️
>
> Marking a message as read using the `markAppInboxAsRead` method does not trigger a tracking event for opening the message. To track an opened message, you need to call the `SendsayPlugin().trackAppInboxOpened` method). 
...
Use the `SendsayPlugin().trackAppInboxOpened(MessageItem)` method to track the opening of App Inbox messages.
...
If you want to ignore tracking consent, use `SendsayPlugin().trackAppInboxOpenedWithoutTrackingConsent` instead. This method will track the event regardless of consent.

#### Track clicked App Inbox message action

Use the `SendsayPlugin().trackAppInboxClick(MessageItemAction, MessageItem)` method to track action invocations in App Inbox messages.
...
If you want to ignore tracking consent, use `SendsayPlugin().trackAppInboxClickWithoutTrackingConsent` instead. This method will track the event regardless of consent.

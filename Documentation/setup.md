...
The Sendsay Flutter SDK can be installed or updated through a dependency in your app's [pubspec](https://docs.flutter.dev/tools/pubspec). [CocoaPods](https://cocoapods.org/) is required to set up the iOS app.

> 📘
>
> Refer to https://github.com/sendsay/sendsay-flutter-sdk/releases for the latest Sendsay Android SDK release.

### Add dependency

In your project's `pubspec.yaml` file, add a dependency to the Sendsay Flutter SDK under `dependencies:`:
...
To resolve the Sendsay SDK dependencies for the iOS app, first `cd` into the `ios` directory in your project:
...
final configuration = SendsayConfiguration(
  ...
  // default baseUrl value is https://api.sendsay.com
...
Flutter application code can be reloaded without restarting the native application itself. This speeds up the development process, but it also means that native code usually continues to run as if nothing happened. You should configure the SDK only once. When developing with hot reload enabled, you should check `SendsayPlugin().isConfigured()` before configuring the SDK.
...
The SDK can be further configured by setting additional properties of the `SendsayConfiguration` object. For a complete list of available configuration parameters, refer to the [Configuration](https://documentation.bloomreach.com/engagement/docs/flutter-sdk-configuration) documentation.
